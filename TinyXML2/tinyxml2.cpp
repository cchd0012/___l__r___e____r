...
static FILE* callfopen_w(const wchar_t* filepath, const wchar_t* mode)
{
    TIXMLASSERT(filepath);
    TIXMLASSERT(mode);
#if defined(_MSC_VER) && (_MSC_VER >= 1400 ) && (!defined WINCE)
...
XMLError XMLDocument::LoadFileW(const wchar_t* filename)
{
    ...
    Clear();
    FILE* fp = callfopen_w(filename, L"rb");
    if (!fp) {
        SetError(XML_ERROR_FILE_NOT_FOUND, 0, "filename=%s", filename);
        return _errorID;
    }
...
XMLError XMLDocument::SaveFileW(const wchar_t* filename, bool compact)
{
    if (!filename) {
        TIXMLASSERT(false);
        SetError(XML_ERROR_FILE_COULD_NOT_BE_OPENED, 0, "filename=<null>");
    ...
    FILE* fp = callfopen_w(filename, L"w");
    if (!fp) {
        SetError(XML_ERROR_FILE_COULD_NOT_BE_OPENED, 0, "filename=%s", filename);
        return _errorID;
    }
